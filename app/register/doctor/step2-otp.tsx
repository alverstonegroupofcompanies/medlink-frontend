...
export default function DoctorOtpScreen() {
  ...
  const handleVerifyOtp = async (otpToVerify?: string) => {
    ...
    setLoading(true);
    try {
      const response = await API.post('/doctor/registration/verify-otp', {
        email,
        otp: otpValue,
      });
      ...
      if (response.data.status) {
        ...
        router.push('/register/doctor/step3-details');
      } else {
  ...
  const handleResendOtp = async () => {
    setResending(true);
    try {
      const response = await API.post('/doctor/registration/send-otp', { email });
      if (response.data.status) {
